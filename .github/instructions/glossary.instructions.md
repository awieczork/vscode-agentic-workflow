...
This file defines canonical vocabulary for all AI agent artifacts in this repository. The governing principle is: one term per concept — aliases fragment understanding and create inconsistency across files. Use `<vocabulary_rules>` for usage obligations, then reference `<canonical_terms>` for definitions and prohibited aliases. Consult `<validation>` for automated detection of non-canonical usage.
...
Synonyms fragment coherent understanding — "error", "failure", and "exception" used interchangeably cause redundant handling or missed connections. Canonical terms enable pattern matching across the entire codebase. Users naturally use varied vocabulary, so the agent must interpret aliases correctly while normalizing its own output. Conflict entries prevent misapplication of terms that share semantic space (e.g., `constraint` vs `boundary`).

</justification>

<benefit>

The agent builds coherent vocabulary without disambiguation overhead and selects precise terms by consulting documented distinctions.
