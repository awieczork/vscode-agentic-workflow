...
|---|---|---|---|---|
| `applyTo` | string | no | VS Code glob pattern or comma-separated patterns. Triggers file-based discovery | `'**/*.ts'`, `'src/api/**'`, `'**/*.ts,**/*.tsx'` |
| `description` | string | no | Semantic matching phrase. Formula: "{verb} {scope} {concern}". Max 1024 chars | `'Enforces TypeScript type-safety conventions'` |
| `name` | string | no | Display name. Rarely needed — omit unless disambiguation is required | `'typescript-conventions'` |
...
| Mode | Frontmatter | Activation | Use when |
|---|---|---|---|
| File-triggered | `applyTo` only | Auto-attaches when matching files are open | Rules apply to specific file types or directories regardless of task |
| On-demand | `description` only | Semantic matching to user prompts | Rules are domain-specific and only relevant when the user asks about that domain |
| Both | `applyTo` + `description` | File match OR semantic match | **Recommended default.** Rules are scoped to files but also relevant to domain questions |
| Manual | No frontmatter | Only when user explicitly references the file | Internal instructions consumed by other artifacts, not meant for automatic discovery |
