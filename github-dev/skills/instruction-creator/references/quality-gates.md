...
|---|---|---|
| Identity prose ("You are a...", "As a...") | P1 | Instructions are ambient constraints, not agent personas |
| Drive letters or absolute OS paths | P1 | `C:\`, `e:\`, `/home/`, `/Users/` — use relative paths only |
| Workflow or step tags (`<workflow>`, `<step_N_verb>`) | P1 | Workflow structure belongs in skills, not instructions |
| Stance words (should, prefer, consider) | P2 | Use NEVER/ALWAYS — binary enforcement, no ambiguity |
| Hedge language (try to, when possible, generally) | P2 | State rules directly without hedging |
| Generic XML group names (`<rules>`, `<guidelines>`, `<best_practices>`) | P2 | Name groups after the specific concern they address |
| Prompt variables (`{{language}}`, `{framework}`) | P2 | Instructions are static — no templating |
| Duplicating linter rules | P2 | Focus on conventions tools cannot check |
| Motivational or filler phrases (Let's, Great job, Remember to) | P3 | State rules directly |
| Temporal language (currently, recently, new) | P3 | Content becomes stale — use timeless phrasing |
...
Anti-patterns specific to instruction artifacts. Each entry describes what is wrong and why.
