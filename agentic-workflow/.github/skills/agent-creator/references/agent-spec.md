...
- **`mcp-servers`** (string[], optional) — MCP server names to enable. Example: `['github', 'filesystem']`
...
- **Status codes** (`COMPLETE` / `BLOCKED` / `PARTIAL`) — Never override — orchestrator depends on these
- **Session ID echo** — Never override — required for tracking
- **Output template structure** — Override only to add domain-specific sections
- **Workflow step format** — Override only when domain steps differ significantly
- **Constraint bullet style** — Override only to add domain-specific constraints
