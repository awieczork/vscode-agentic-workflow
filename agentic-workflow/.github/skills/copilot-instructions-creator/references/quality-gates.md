...
- **Coding standards or style rules** (P2) — Fix: Move to a `.instructions.md` file scoped to the relevant language
- **Agent pool listings (`@planner`, `@developer`)** (P1) — Fix: Move to `brain.agent.md` — copilot-instructions.md is agent-agnostic
- **Verbose project descriptions (>3 sentences)** (P2) — Fix: Condense to project name + tech stack in 1–3 sentences
- **Framework or orchestration metadata** (P2) — Fix: Remove entirely — implementation details do not belong here
- **Stale commands or paths** (P2) — Fix: Verify each command runs successfully or remove it
- **Hedge language (try to, when possible, generally)** (P2) — Fix: Rewrite as NEVER/ALWAYS — binary enforcement only
- **Temporal language (currently, recently, new)** (P3) — Fix: Use timeless phrasing — content becomes stale
- **Motivational filler (Let's, Great job, Remember to)** (P3) — Fix: State rules directly without filler

</banned_patterns>

<copilot_instructions_anti_patterns>

Anti-patterns specific to copilot-instructions.md artifacts. Each entry describes what's wrong and why.
